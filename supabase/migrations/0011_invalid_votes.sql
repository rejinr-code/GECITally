-- Invalid / spoilt ballots count toward the round size and votes polled, not toward a candidate.

alter table public.count_rounds
  add column if not exists invalid_votes integer not null default 0;

alter table public.count_rounds
  drop constraint if exists count_rounds_invalid_votes_nonnegative;

alter table public.count_rounds
  add constraint count_rounds_invalid_votes_nonnegative check (invalid_votes >= 0);
  ...
  begin
    if v_rejected_id is not null then
      ...
      insert into public.count_rounds (
        post_id, staff_id, round_number, status, verified_by, verified_at, invalid_votes
      )
      values (
        p_post_id,
        v_staff,
...
grant execute on function public.submit_count_round(uuid, jsonb, integer) to authenticated;
revoke execute on function public.submit_count_round(uuid, jsonb, integer) from public, anon;
