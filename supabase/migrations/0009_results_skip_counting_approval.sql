-- If public results do not wait for a supervisor, counting must not wait either.
...
declare
  ...
  v_require_review boolean;
begin
  v_require_review := coalesce(new.counting_require_verification, true)
    and coalesce(new.results_require_verification, true);
...
create trigger trg_accept_pending_on_counting_mode
  after update of counting_require_verification, results_require_verification on public.elections
