-- Candidate 1st/2nd (and further) marks, matching invalid_slot_votes.
-- Single-seat posts still store one total; slot_votes is [votes].

alter table public.count_entries
  add column if not exists slot_votes integer[] not null default '{}';

comment on column public.count_entries.slot_votes is
  'Votes for this candidate by ballot mark (1st, 2nd, ...). Empty means a legacy total-only row.';
