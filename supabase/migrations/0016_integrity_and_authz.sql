-- Assigned Counting Supervisors can read peer rounds on their posts (progress = post-wide).
-- Live results RPC is limited to election roles. Multi-seat rounds must balance per mark.
...
create policy "rounds_select" on public.count_rounds
  for select using (
    ...
    or public.is_assigned_to_post(post_id)
  );
  ...
  for select using (
    ...
    or exists (
      ...
      where cr.id = round_id
        and (
          cr.staff_id = auth.uid()
          or public.is_assigned_to_post(cr.post_id)
        )
    )
  );
...
begin
  ...
  if (
    ...
    raise exception 'Duplicate candidate in this round';
  end if;
  ...
  if v_seats > 1 then
    if exists (
      select 1
      from jsonb_array_elements(p_entries) e
      where jsonb_typeof(e->'slot_votes') is distinct from 'array'
         or coalesce(jsonb_array_length(e->'slot_votes'), 0) <> v_seats
    ) then
      raise exception 'Submit 1st and 2nd counts for each candidate';
    end if;
    if exists (
      select 1
      from jsonb_array_elements(p_entries) e
      where exists (
        select 1
        from jsonb_array_elements(e->'slot_votes') s
        where (s #>> '{}') is null
           or (s #>> '{}') ~ '[^0-9]'
      )
      or (
        select coalesce(sum((s #>> '{}')::int), 0)
        from jsonb_array_elements(e->'slot_votes') s
      ) <> (e->>'votes')::int
    ) then
      raise exception 'Candidate 1st/2nd counts must add up to that candidate''s total';
    end if;
  end if;
  ...
  if v_seats > 1 then
    for v_slot_index in 0 .. v_seats - 1 loop
      select coalesce(sum((e->'slot_votes'->v_slot_index #>> '{}')::int), 0)
        into v_slot_total
      from jsonb_array_elements(p_entries) e;
      v_slot_total := v_slot_total + coalesce(v_slots[v_slot_index + 1], 0);
      if v_slot_total <> v_round_ballots then
        raise exception 'Each ballot mark (1st, 2nd, ...) plus its invalids must add up to % ballots', v_round_ballots;
      end if;
    end loop;
  end if;
  ...
  loop
    insert into public.count_entries (round_id, candidate_id, votes, slot_votes)
    values (
      v_round_id,
      (v_entry->>'candidate_id')::uuid,
      (v_entry->>'votes')::int,
      case
        when jsonb_typeof(v_entry->'slot_votes') = 'array' then
          array(select jsonb_array_elements_text(v_entry->'slot_votes')::int)
        when v_seats = 1 then
          array[(v_entry->>'votes')::int]
        else
          '{}'::integer[]
      end
    );
